...
- fast CLI commands (e.g. `cargo test`)
...
- MCP end-to-end flows (Claude Desktop connecting to local MCP server)
- Tauri UI integration (app window, button interactions, event flows)
- API integration tests (Outline, Obsidian plugin REST API)
- Security validation (path traversal, symlink attacks, rate limits)
- Port conflict and reassignment scenarios
- Audit log correctness (entries written, events emitted)
- Cross-platform behavior (Windows vs macOS path handling)
- Performance sanity checks (large directory listing, 100k file vault)
...
- commands executed
- environment used (Windows/macOS, Tauri dev/release)
- test output summary
- pass/fail status
